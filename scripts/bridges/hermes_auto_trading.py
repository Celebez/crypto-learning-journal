...
RULES = {
    "entry": {
        ...
        "volume_surge_ratio": 1.2,    # Volume > 1.2x average = confirmation
        "min_score": 4,               # Minimum entry score (out of 7) — lebih ketat
    ...
    "exit": {
        ...
        "max_hold_hours": 48,         # Max hold time (2 hari, bukan 3)
        "trailing_stop_pct": 1.2,     # Trailing stop: 1.2% from peak (lebih ketat)
        "partial_close_pct": 50,      # Close 50% when +2%, sisanya trailing
    },
    # --- RISK MANAGEMENT ---
    "risk": {
        "max_positions": 10,          # Max open positions (dari 15 → 10)
        "max_per_position_usd": 75,   # Max USD per position (dari 100 → 75)
        "max_total_exposure_pct": 60, # Max 60% of equity (dari 80% → 60%)
        "min_balance_usd": 300,       # Keep at least $300 free (dari 200 → 300)
    ...
    "symbols": {
        ...
        "exclude": ["DOGEUSDT"],  # Never scan/trade these tokens
    },
    # --- ANALYSIS TIMEFRAMES ---
    "timeframes": {
...
def analyze_symbol(session, symbol: str, interval: int = 15, limit: int = 100) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "macd": {"value": macd_val, "signal": signal_val, "histogram": hist_val, 
                     "bullish": macd_bullish, "cross_up": macd_cross_up},
            "bb": {"width": bb_width_val, "squeeze": bb_squeeze, "position": bb_position,
                   "upper": float(bb_upper.iloc[-1]), "lower": float(bb_lower.iloc[-1])},
            "ema": {"fast_above_slow": ema_bullish, "ema7": float(ema7.iloc[-1]), "ema20": float(ema20.iloc[-1])},
...
def compute_exit_score(analysis: Dict, entry_price: float, current_price: float, 
                       peak_price: float, hold_hours: float) -> Tuple[int, List[str], str]:
    ...
    # 3. Trailing Stop
    if drawdown_from_peak >= rules["trailing_stop_pct"] and pnl_pct > 0:
        score += 2
        reasons.append(f"Trailing stop: -{drawdown_from_peak:.1f}% from peak")
        action = "CLOSE"
    
    # 4. RSI Overbought
    if analysis["rsi"] > rules["rsi_overbought"]:
        score += 1
        reasons.append(f"RSI overbought ({analysis['rsi']:.1f})")
        if score >= 2:
            action = "CLOSE"
    ...
    if hold_hours >= rules["max_hold_hours"]:
        score += 1
        reasons.append(f"Max hold time ({hold_hours:.0f}h)")
        if score >= 2:
            action = "CLOSE"
    
    # 6. MACD Bearish Cross
    if not analysis["macd"]["bullish"] and analysis["macd"]["histogram"] < 0:
        score += 1
        reasons.append("MACD bearish")
    ...
    for sym in symbols:
        try:
            ...
            if "error" not in analysis:
                ...
                if score >= min_score and short_score >= 4:
                ...
                elif score >= min_score - 1 or short_score >= 3:
                    signal = "WATCH"
                else:
                    signal = "PASS"
...
def monitor_positions(session) -> List[Dict]:
    ...
    for pos in portfolio["positions"]:
        try:
            ...
            # Compute exit score
            # For peak price, use resistance level from analysis as approximation
            ...
            pnl_pct = ((pos["mark"] - pos["entry"]) / pos["entry"]) * 100 if pos["entry"] > 0 else 0
